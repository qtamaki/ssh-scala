...
//    }
     ...
     * @return this
     */
    def putBytes(b: Array[Byte]):T = putBytes(b, 0, b.length)
     ...
     * @return this
     */
    def putBytes(b: Array[Byte], off:Int, len:Int):T = {
        val x = putUInt32(len - off)
        x.putRawBytes(b, off, len).asInstanceOf[T]
    }
    ...
    def putRawBytes(d:Array[Byte]):T ={
        putRawBytes(d, 0, d.length);
    }

    def putRawBytes(d: Array[Byte], off:Int, len:Int):T = {
        ensureCapacity(len);
    ...
    def putString(str:Array[Byte]):T = {
        return putBytes(str);
    }

    def putString(str:Array[Byte], offset:Int, len:Int):T = {
        return putBytes(str, offset, len)
    }

    def putString(string:String):T = {
     ...
     * @return this
     */
    def putSensitiveString(str:Array[Char]):T = {
    ...
    def putPublicKey(key:PublicKey):T = {
        KeyType.fromKey(key).putPubKeyIntoBuffer(key, this);
        return this.asInstanceOf[T]
    }

    def putSignature(sigFormat:String, sigData:Array[Byte]):T = {
